...
TEST_F(AgentTest, UniquePartnersSuccess)
{
    ScAddr c1, c2, p1, p2, p3, p4;
    init_data(m_ctx.get(), c1, c2, p1, p2, p3, p4);
    
    m_ctx->SubscribeAgent<FindUniquePartnersAgent>();
    ...
    ScAction action = m_ctx->GenerateAction(PartnersKeynodes::action_find_unique_partners);
    action.SetArguments(args);
    ...
    // c1: {p1, p2, p3}, c2: {p2, p3, p4}
    // Уникальные для c1: {p1} - только 1 партнёр
    EXPECT_EQ(count, 1u);
    EXPECT_TRUE(m_ctx->CheckConnector(res, p1, ScType::ConstPermPosArc));
    
    m_ctx->UnsubscribeAgent<FindUniquePartnersAgent>();
}

TEST_F(AgentTest, UniquePartnersEmpty)
{
    ScAddr c1, c2, p1, p2, p3, p4;
    init_data(m_ctx.get(), c1, c2, p1, p2, p3, p4);
    ...
    // Создаём аргументы: c2 относительно c1
    ScAddr args = m_ctx->GenerateNode(ScType::ConstNode);
    ScAddr a1 = m_ctx->GenerateConnector(ScType::ConstPermPosArc, args, c2);
    m_ctx->GenerateConnector(ScType::ConstPermPosArc, PartnersKeynodes::rrel_1, a1);
    ScAddr a2 = m_ctx->GenerateConnector(ScType::ConstPermPosArc, args, c2);
    m_ctx->GenerateConnector(ScType::ConstPermPosArc, PartnersKeynodes::rrel_2, a2);
    
    // c2 относительно самой себя - все партнёры общие
    ScAction action = m_ctx->GenerateAction(PartnersKeynodes::action_find_unique_partners);
    action.SetArguments(args);
